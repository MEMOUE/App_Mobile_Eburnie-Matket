...
class _DetailMagasinScreenState extends State<DetailMagasinScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        slivers: [
          // ── Hero avec coordonnées intégrées ──
          _buildAppBar(m),
          SliverToBoxAdapter(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                // Chips rapides (nb annonces, marché, stand)
                _buildInfoCard(m),
  ...
  // ── AppBar avec coordonnées dans le hero ──────────────────────────────────

  Widget _buildAppBar(Magasin m) {
    return SliverAppBar(
      // Hauteur élargie pour accueillir les chips de contact
      expandedHeight: 185,
      pinned: true,
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      // Nom + catégorie + chips (ville, marché, vérifié)
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                              ...
                              children: [
                                if (m.villeDisplay.isNotEmpty)
                                  _HeroChip(
                                    ...
                                    label: m.villeDisplay,
                                  ),
                                if (m.marcheDisplay?.isNotEmpty ?? false)
                                  _HeroChip(
                                    icon: Icons.storefront_rounded,
                                    label: m.marcheDisplay!,
                                  ),
                                if (m.isVerified)
                                  _HeroChip(
                                    icon: Icons.verified,
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(
                child: ElevatedButton.icon(
                  onPressed: magasin.qrCodeAbsoluteUrl != null
                      ? onDownload
                      : null,
...
/// Chip utilisé dans le hero (fond semi-transparent blanc)
